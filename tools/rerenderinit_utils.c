...
/*   Updated: 2025/09/09 11:41:24 by abdael-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rerenderinit_utils(t_globaldata *t, double *ray_angle, int line_index)
{
	...
	step_texture_step = (double)t->wrapper[texture_index].height / (double)perp_wall_on_screen;
	tex_pos = (start_draw - WIN_HEIGHT / 2.0 + perp_wall_on_screen / 2.0) * step_texture_step;
	tindex = start_draw - 1;
	while (++tindex < end_draw)
	{
		tex_y = (int)tex_pos;
		tex_pos += step_texture_step;
		my_mlx_pixel_put(&t->img, line_index, tindex,
				get_texture_color(&t->wrapper[texture_index], tex_x, tex_y));
	}
}
